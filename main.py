...
# ==========================================
# NUEVO: CONFIGURACIÓN DEL FRONTEND (CARPETA STATIC)
# ==========================================
...
# ==========================================
# 5. HISTORIAL  PARA MULTI-USUARIO)
# ==========================================
...
def obtener_historial(
    ...
    # 1. Unimos Movimientos con Proveedor Y filtramos por el dueño (owner_id)
    query = db.query(models.Movimiento).join(models.Proveedor).filter(
        models.Proveedor.owner_id == current_user.id # <-- ¡EL FILTRO VITAL!
    )
...
def eliminar_movimiento(mov_id: int, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    # Buscamos el movimiento asegurándonos que el proveedor asociado sea del usuario actual
    mov = db.query(models.Movimiento).join(models.Proveedor).filter(
        models.Movimiento.id == mov_id,
        models.Proveedor.owner_id == current_user.id
    ).first()
    ...
    if mov: 
        ...
        db.commit()
        return {"status": "success"}
    raise HTTPException(status_code=404, detail="Movimiento no encontrado o sin permisos")

@app.post("/marcar_pagado/{mov_id}")
def marcar_movimiento_pagado(mov_id: int, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    # Buscamos el movimiento asegurándonos que el proveedor asociado sea del usuario actual
    mov = db.query(models.Movimiento).join(models.Proveedor).filter(
        models.Movimiento.id == mov_id,
        models.Proveedor.owner_id == current_user.id
